...
class MessageController {
    ...
    try {
      ...
      if (!isUser && !isAgent)
        return res.status(403).json({
          message:
            "vous n'êtes pas authorisé(s) à effectuer cette requête: Mot de passe ou email incorrects",
          status: false,
        });
      const isEntreprise = await Entreprise.findOne({
        _id: entreprise,
        statut: 1,
      });
      if (!isEntreprise)
        return res.status(403).json({
          message:
            "vous n'êtes pas authorisé(s) à effectuer cette requête: Vous n'êtes pas membre de l'entreprise",
          status: false,
        });
      if (
        !Array.isArray(req.body.groupe) &&
        typeof req.body.groupe === "string"
      )
      ...
      if (req.body.groupe && req.body.groupe.length) {
        ...
        if (!myGroupes.length)
          return res
            .status(403)
            .json({ message: "Vous ne disposez aucun groupe.", status: false });
        myGroupes.map((item) => idCollection.push(item._id.toString()));
        newCollections = req.body.groupe.filter((item) =>
          idCollection.includes(item)
        );
        if (!newCollections.length)
          return res.status(403).json({
            message: "Les contacts ou groupe n'existent pas",
            status: false,
          });
        newCollections.map((item) =>
          idObjetCollection.push(new mongoose.Types.ObjectId(item))
        );
        ...
        if (!myContacts.length)
          return res.status(403).json({
            message: "Vous ne disposez aucun contact.",
            status: false,
          });
        myContacts.map((item) => idCollection.push(item._id.toString()));
        newCollections = req.body.contact.filter((item) =>
          idCollection.includes(item)
        );
        if (!newCollections.length)
          return res.status(403).json({
  ...
  static async getAll(req, res) {
    try {
      ...
      if (isMember) {
        isStructure = await Entreprise.findOne({
          _id: isMember.entreprise,
          statut: 1,
        });
        if (isStructure)
          resultat = await Message.find({
            entreprise: isStructure._id,
            statut: 1,
          })
            .populate("entreprise")
            .populate("user")
      ...
      if (!resultat.length)
        return res
          .status(203)
          .json({ message: "Aucun contact trouvé.", status: false });
  ...
  static async getById(req, res) {
    try {
      ...
      if (isUser || isAgent) {
        ...
        isMessage = await Message.findOne({
          _id: req.params.id,
          entreprise,
          statut: 1,
        })
      ...
      if (!isMessage)
        return res
          .status(402)
          .json({ message: "Ce message n'existe pas.", status: false });
      res.status(202).json({
        message: "Requête traitée avec succès.",
        status: true,
        data: isMessage,
  ...
  static async getByName(req, res) {
    try {
      ...
      if (isUser || isAgent) {
        ...
        isMessage = await Message.find({ entreprise, statut: 1 }).populate(
          "entreprise"
        );
      } else {
        isCompagny = await Plateforme.findOne({ _id: plateforme, statut: 1 });
      ...
      isMessage = isMessage.filter(
        (item) =>
          item.object.includes(req.params.object) &&
          item.entreprise._id === entreprise
      );
      if (!isMessage.length)
        return res
          .status(402)
  ...
  static async update(req, res) {
    try {
      ...
      if (!hasMessage)
        return res.status(403).json({
          message: "Le message à modifier n'est pas disponible.",
          status: false,
        });
  ...
  static async delete(req, res) {
    try {
      ...
      if (!isMessage)
        return res.status(402).json({
          message: "Le message à modifier n'existe pas.",
          status: false,
        });
      delete req.body._id;
      for (const key in req.body) {
      ...
      if (!updated.acknowledged || !updated.modifiedCount)
        return res
          .status(203)
          .json({ statut: false, message: "Suppression non effectuée." });
      res.status(202).json({
        status: true,
  ...
  static async verifyEmail(req, res) {
    try {
      ...
      if (user)
        return res
          .status(400)
          .json({ message: "Ce compte est déjè utilisé.", statut: false });
      ...
      const connection = transporteur({
        ...
        pass: `${plateforme[0].passwordEmailInfo}`,
      });
      ...
      if (!email.response.includes("OK"))
        return res
          .status(400)
          .json({ message: "Connexion interrompue.", statut: false, error });
      const validate = await ValidateCode.create(req.body);
      // Sinon on retourne le code de validation au client.
      return res.status(201).json({
        message: "Code de validation unique.",
        code: validate.code,
        data: validate,
        statut: true,
      });
  ...
  static async createEmail(req, res) {
    try {
      ...
      if (
        !(
          (Array.isArray(contact) &&
            contact.some((emailAdress) => verify_email_adress(emailAdress))) ||
          (typeof contact == "string" && verify_email_adress(contact))
        )
      )
        return res.status(400).json({
          message: "Les contacts chargés ne contiennent aucun adresse mail.",
          status: false,
        });
      ...
      if (
        ...
        contact = contact.filter((item) => verify_email_adress(item));
      ...
      if (canal != "email")
        return res.status(404).json({
          status: false,
          message: "Impossible de poursuivre cette requette.",
        });
      ...
      const connection = transporteur({
        user: "nfcdjobo",
        pass: "y f m d s f z e e t s g t t j f",
      });
      const attachements = [];
      if (req.files) {
        req.files.map((piece) => {
      ...
      return res
        .status(201)
        .json({ message: "Message transféré avec succès !", statut: true });
    } catch (e) {
      console.log(e);
    ...
    try {
      ...
      if (contact.length) {
        ...
        groupe.map(async (item) => {
          const ThisSomeGroupe = await Groupe.findOne({
            _id: item._id,
            entreprise,
            statut: 1,
          }).populate("contact");
      ...
      ThisContactsOrGroupe.forEach((adress) => {
        SendMessageBusness(
          "+15551335562" /**verifCompagny.whatsapp*/,
          adress,
          FindMessage.contenu
        );
      });
