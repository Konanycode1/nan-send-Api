...
class UserController {
    ...
    static async delete(req , res){
        try {
            ...
            res.status(201).json({message: "Suppression effectué avec succès", data: updated, status: true});            
        } catch (error) {
    ...
    static async destroy(req , res){
        try {
            ...
            const user = await User.findOne({email, statut: 1});
            if(!user) return res.status(203).json({message: "L'utilisateur à modifier n'existe pas !", status: false});
            if(!user.entreprise) await User.deleteOne({_id: user._id, email, statut: 1});
