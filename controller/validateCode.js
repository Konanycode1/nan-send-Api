...
class ValidateCodeController {
  static async getByIdAndCode(req, res) {
    ...
    if (!isValideCode)
      return res.status(203).json({ message: "Email expiré.", status: false });
  ...
  static async deleteIfExpires(req, res) {
    try {
      ...
      if (isValideCode && !isUser && isValideCode.expireIn > Date.now()) {
        ...
        isValideCode &&
        !isUser &&
        isValideCode.expireIn <= Date.now()
      ) {
        deleted = await ValidateCode.deleteOne({ email, code });
        res
          .status(203)
          .json({ message: "register user", status: false, data: deleted });
      } else if (
        isValideCode &&
        isUser &&
        isValideCode.expireIn <= Date.now()
        ...
        res
          .status(203)
          .json({ message: "location user", status: true, data: [0] });
      }
    } catch (error) {
  ...
  static async deleteExpired(req, res) {
    ...
    if (fetchAll.expireIn < Date.now())
      await ValidateCode.deleteOne({ _id: fetchAll._id });
